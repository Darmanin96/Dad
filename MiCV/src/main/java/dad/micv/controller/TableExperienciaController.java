...
public class TableExperienciaController implements Initializable {
    ...
    void onAñadirAction(ActionEvent event) {
        ...
        if (tituloDialogController.isConfirmar()){
            String denominacion = tituloDialogController.getDenominacion().getText();
            String empleador = tituloDialogController.getOrganizador().getText();
            LocalDate desde = tituloDialogController.getDesde().getValue();
            LocalDate hasta = tituloDialogController.getHasta().getValue();
            Experiencia experiencia1 = new Experiencia(denominacion, empleador, desde, hasta);
    ...
    void onEliminarAction(ActionEvent event) {
        Experiencia experienciaSeleccionada = tableExperiencia.getSelectionModel().getSelectedItem();
        if (experienciaSeleccionada != null){
            experiencia.remove(experienciaSeleccionada);
        }
    }
