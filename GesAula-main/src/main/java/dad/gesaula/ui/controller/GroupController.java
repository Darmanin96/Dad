...
public class GroupController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ExamenesSlider.valueProperty().addListener((observable, oldValue, newValue) -> {
            ExamLabel.setText(String.format("%.0f%%", newValue.doubleValue()));
        });

        ActitudSlider.valueProperty().addListener((observable, oldValue, newValue) -> {
            ActitudLabel.setText(String.format("%.0f%%", newValue.doubleValue()));
        });

        PracticasSlider.valueProperty().addListener((observable, oldValue, newValue) -> {
            PracticasLabel.setText(String.format("%.0f%%", newValue.doubleValue()));
        });
    }
